...
export default function AdminPage() {
  ...
  useEffect(() => { if (authed && filter !== "comments") fetchStories(); }, [authed, filter]);
  ...
  const css = `
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .admin-header h1 { font-size: 28px; font-weight: 700; letter-spacing: -0.02em; margin-bottom: 4px; }
    .admin-header p { color: var(--gray); font-size: 14px; }
    .login-card { max-width: 400px; margin: 120px auto; background: white; padding: 40px; border-radius: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.06); border: 1px solid var(--border); text-align: center; }
    ...
    .login-input { width: 100%; padding: 14px 18px; border: 2px solid var(--border); border-radius: 12px; font-size: 15px; font-family: var(--font); margin-bottom: 14px; background: var(--bg); }
    .login-input:focus { outline: none; border-color: var(--blue); }
    .login-btn { width: 100%; padding: 14px; background: var(--blue); color: white; border: none; border-radius: 12px; font-size: 15px; font-weight: 700; cursor: pointer; font-family: var(--font); }
    .login-btn:hover { background: var(--blue-dark); }
    .tabs { display: flex; gap: 6px; margin-bottom: 8px; flex-wrap: wrap; }
    .tab { padding: 8px 18px; border-radius: 100px; font-size: 13px; font-weight: 600; border: 1.5px solid var(--border); background: white; cursor: pointer; font-family: var(--font); color: var(--gray); transition: all 0.2s; }
    ...
    .story-row { background: white; border-radius: 16px; padding: 24px; margin-bottom: 14px; border: 1px solid var(--border); transition: box-shadow 0.2s; }
    ...
    .story-tag { font-size: 11px; font-weight: 700; padding: 4px 10px; border-radius: 100px; text-transform: uppercase; letter-spacing: 0.04em; }
    ...
    .admin-nav { display: flex; justify-content: space-between; align-items: center; padding: 16px 48px; border-bottom: 1px solid var(--border); background: white; }
    .admin-nav-logo { font-family: 'League Spartan', var(--font); font-size: 28px; font-weight: 800; color: var(--black); letter-spacing: -0.03em; }
    .admin-nav-badge { font-family: var(--font); font-size: 12px; font-weight: 600; background: var(--blue-pale); color: var(--blue); padding: 4px 10px; border-radius: 100px; }
    ...
    .btn { padding: 10px 20px; border-radius: 10px; font-size: 13px; font-weight: 600; cursor: pointer; font-family: var(--font); transition: all 0.2s; border: none; }
    ...
    .queue-info { background: var(--blue-pale); border: 1px solid var(--blue-light); border-radius: 12px; padding: 14px 18px; margin-bottom: 20px; font-size: 14px; color: var(--blue-dark); display: flex; justify-content: space-between; align-items: center; }
    ...
    return (
      <div><style>{css}</style>
        <div className="login-card">
          <h1><span style={{ fontFamily: "'League Spartan', var(--font)", fontSize: 28, fontWeight: 800, letterSpacing: "-0.03em" }}>Date&Tell</span></h1>
                  ...
                  {editing === story.id ? (
                    ...
                    <>
                      ...
                      <div className="story-actions">
                        ...
                        {filter === "published" && (
                          <button className="btn btn-delete" onClick={() => { if (confirm("Delete this published story?")) updateStatus(story.id, "deleted"); }} disabled={actionLoading === story.id}>{actionLoading === story.id ? "..." : "Delete"}</button>
                        )}
                        {filter === "reported" && (
